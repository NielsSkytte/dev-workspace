...
### Never `fab import` an item that belongs in a git-connected workspace

Fabric git matches workspace items to repo items by **`logicalId`**, which is minted
when an item is *first committed to git*. An item created by `fab import` gets its own
identity and is invisible to git — so when the same item then arrives from the repo,
Fabric sees a **second item with the same display name** and Update from git fails with
a duplicate error. Verified on Carl Ras 2026-08-10: seven items duplicated across two
workspaces; the only fix was deleting every workspace copy and re-updating from git.

**The `logicalId` in a hand-written `.platform` IS honoured** when the item arrives via
git. The duplicate is caused purely by the item pre-existing outside git.

**Routine for any item destined for a repo:**
1. Author the files locally -> commit -> push -> **Update from git**. Do *not* `fab import`.
2. `fab import` is for items that will never be in the repo, and for **updating an item
   that already exists** (`-f` updates in place, keeps the item id and folder).
3. Creating a notebook via `fab import` also needs `--format .py`, or the service tries
   to parse `notebook-content.py` as `.ipynb` JSON and fails with `InvalidNotebookContent`.

### Expect one normalising commit per hand-authored item

Fabric re-serialises every item definition on import, so a hand-authored file shows as
`Modified` in git status until it has been committed **from the workspace once**. Measured
on Carl Ras 2026-08-10 — four distinct normalisations:

| File | Change | Lossy |
|---|---|---|
| `.platform` | trailing newline removed (`logicalId` preserved) | no |
| `alm.settings.json`, `xmla.json`, `.sqlproj` | LF -> CRLF | no |
| `pipeline-content.json` | JSON **key reordering** (`typeProperties` before `name`, `secureInput` before `secureOutput`) | no |
| `notebook-content.py` | **markdown cells mangled** — blank `#` lines collapsed into `# #` | **yes** |

Only the last loses anything, and it degrades notebook documentation. Keep markdown cells
free of blank comment lines, or accept that the first workspace commit will reflow them.

This churn is **one-time per item**, not recurring. Tell the user to expect it rather than
...
`UnicodeEncodeError`. Prefix calls with `PYTHONIOENCODING=utf-8`. This is the CLI's *output* —
unrelated to the ASCII-only rule for file content.
