...
11. **Tenant-scoped credentials** — before running any tenant-bound CLI (`fab`, `az`, `pac`, the Dataverse discovery scripts) in a customer project, verify the authenticated tenant against that customer node's `tenant_id:`. Never inherit whatever session was last authenticated. One machine carries guest (`ext_`) identities in several customer tenants and the last-used one is arbitrary, so a wrong-tenant listing is worse than no listing — it looks like an answer. Check first, as its own step (`fab auth status`, `az account show`); on mismatch **stop**, say so, and hand the user the interactive login (`! fab auth login -t <tenant_id>`) rather than running further commands or reasoning from their output. Recorded on the customer node because every project under a customer shares them (added 2026-07-31 after a `fab ls` in a Matas session returned the Carl Ras estate).
