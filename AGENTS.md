...
11. **Tenant-scoped credentials** — before running any tenant-bound CLI (`fab`, `az`, `pac`, the Dataverse discovery scripts) in a customer project, verify the authenticated tenant against that customer node's `tenant_id:`. Never inherit whatever session was last authenticated. One machine carries guest (`ext_`) identities in several customer tenants and the last-used one is arbitrary, so a wrong-tenant listing is worse than no listing — it looks like an answer. Check first, as its own step (`az account show`; for `fab` see below); on mismatch **stop**, say so, and hand the user the interactive login rather than running further commands or reasoning from their output. Recorded on the customer node because every project under a customer shares them (added 2026-07-31 after a `fab ls` in a Matas session returned the Carl Ras estate).
    ...
    **For `fab`, never call it bare — always go through `ops\bin\fab-as.ps1 <Customer> <args>`** (added 2026-08-04). `fab` keeps a single global identity in `~/.config/fab` and has no profile flag, so a bare `fab` inherits whatever tenant was last used and switching customers meant an interactive re-login every time. The wrapper gives each customer its own fab home (`%LOCALAPPDATA%\fab-profiles\<Customer>`, outside every repo — token caches are secrets), reads `tenant_id:`/`account:` from that customer node's `CLAUDE.md` so there is no second registry to drift, and builds the login for you: `ops\bin\fab-as.ps1 <Customer> login`. Bare `ops\bin\fab-as.ps1` lists every customer with its tenant and whether a login is cached. A customer logs in **once**; after that the tenant check is satisfied by construction and no session needs to switch users. A customer node with no `tenant_id:` is refused, not guessed.
