...
That page also shows "Nytte til stede" = Fakturerbare / Normtimer (a different denominator, 141.00
in August, giving 102.84%). The BONUS runs on faktureringsprocent, not on that one.
...
def report(hours, basis=BASIS, rate=RATE):
    ...
    nxt = [t for t in TIERS if t[0] > pct]
    if nxt:
        nb, _ = min(nxt)
        need = nb * basis
        gain = bonus_kr(need, basis, rate) - bonus_kr(hours, basis, rate)
        out += ["",
                "  next boundary %.0f%% at %.2f h -- %.2f h away, worth %s kr"
                % (nb * 100, need, need - hours, "{:,.0f}".format(gain)),
                "  that is %s kr per hour, IF those hours were actually worked."
                % "{:,.0f}".format(gain / (need - hours)) if need > hours else ""]
    else:
