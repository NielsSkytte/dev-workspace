...
**Implemented 2026-06-22** (commit `3f4ccd4`): the system is live at `ops/memory/` (Storage = `daily/` + `store/`, Injection = SessionStart snapshot, Recall = keyword grep, Evaluation = `type: evaluative`). Canonical build detail lives in `ops/memory/README.md` and `AGENTS.md` > Memory — this record stays the *why/design*, not the *how*. Recall is keyword for now; DB/semantic deferred. See [[skill-usage-evaluation]].
